...
fn get_links_from_site(html: &str) -> HashSet<String> {
    Document::from(html)
...
fn main() -> Result<()>{
    ...
    while !new_urls.is_empty() {
        ...
        println!("Errors: {:#?}", errors.into_iter().map(Result::unwrap_err).collect::<Vec<Error>>())
    }
